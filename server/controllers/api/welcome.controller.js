...
async function overView(req, res, next) {
  ...
  try {
    ...
    if (Object.keys(whereOfUtilization).length === 0) {
      ...
      sanction_releasedFund = await db.query(`SELECT pp.id, p.project_name, pp.project_name AS project_code, pp.allocated_budget, SUM(fr.amount_recieved) AS released_amt FROM project_plan pp LEFT JOIN finance_recieved fr ON pp.id = fr.project_plan_id left JOIN project p on pp.project_name = p.project_code GROUP by pp.id`,
        { type: db.QueryTypes.SELECT });
    ...
    Left JOIN finances table3 ON table1.id = table3.project_plan_id
    GROUP BY table1.id`,
        { type: db.QueryTypes.SELECT });
